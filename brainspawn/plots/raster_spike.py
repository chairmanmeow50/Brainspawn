""" Module for raster spike visualization. Ported from old Java nengo.
"""
...
class RasterSpike(Plot):
    """ Class for raster spike visualization. Extends from Plot.
    """

    WINDOW_SIZE = 100

    def __init__(self, main_controller, obj, cap):
        """ Initializes class, sets title, and labels.
        """
        ...
        self._initial_end_time = 1.0 * \
            self.WINDOW_SIZE / settings.MAX_WINDOW_SIZE
    ...
    def supports_cap(cap):
        """ Supports the 'spikes' capability.
        """
        return cap.name in ["spikes"]

    def update(self, start_step, step_size, spikes):
        """ Plots the spikes with imshow (which produces a rasterized image).
        """
        if self._image:
            self._image.remove()
            self._image = None
        ...
        self._image = self.axes.imshow(
            # interpolation="none" is not supported on cairo
            spikes_view.T, cmap="binary", aspect="auto",
            interpolation="nearest", origin="lower",
            extent=(start_time, end_time, 0, spikes_view.shape[1]))

        self.set_default_xlim(max(end_time, self._initial_end_time),
                              self._initial_end_time)
